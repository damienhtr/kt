...
/**
 * Rest_Reflection_Node
 */
require_once 'classes/rest/reflection/Node.php';
...
/**
 * Rest_Reflection_Prototype
 */
require_once 'classes/rest/reflection/Prototype.php';
 ...
 *
 */
abstract class Rest_Reflection_Function_Abstract
{
    /**
     ...
     *
     * @param Rest_Reflection_Node $parent
     * @param int $level
     * @return void
     */
    protected function _addTree(Rest_Reflection_Node $parent, $level = 0)
    {
        ...
        foreach ($this->_sigParams[$level] as $value) {
            $node = new Rest_Reflection_Node($value, $parent);
    ...
    protected function _buildTree()
    {
        ...
        foreach ((array) $this->_return as $value) {
            $node = new Rest_Reflection_Node($value);
            $this->_addTree($node);
            $returnTree[] = $node;
        }
    ...
    protected function _buildSignatures($return, $returnDesc, $paramTypes, $paramDesc)
    {
        ...
        foreach ($endPoints as $node) {
            ...
            do {
                ...
                $node = $node->getParent();
            } while ($node instanceof Rest_Reflection_Node);
        ...
        foreach ($signatures as $signature) {
            $return = new Rest_Reflection_ReturnValue(array_shift($signature), $this->_returnDesc);
            $tmp    = array();
            foreach ($signature as $key => $type) {
                $param = new Rest_Reflection_Parameter($params[$key], $type, $this->_paramDesc[$key]);
                $param->setPosition($key);
                $tmp[] = $param;
            }

            $this->_prototypes[] = new Rest_Reflection_Prototype($return, $tmp);
        }
    }
    ...
    public function __call($method, $args)
    {
        ...
        require_once 'classes/rest/Exception.php';
        throw new Rest_Exception('Invalid reflection method ("' .$method. '")');
    }
    ...
    public function setDescription($string)
    {
        if (!is_string($string)) {
            require_once 'classes/rest/Exception.php';
            throw new Rest_Exception('Invalid description');
        }
     ...
     * {@link Rest_Reflection_Prototype Rest_Reflection_Prototypes}
     *
     * @return array
     */
